...
namespace SeaPeaYou.PeaPdf
{
    ...
    class PdfFile
    {
        ...
        public PdfDict FileTrailerDict => xRef.Dict;
        ...
        public PdfFile(byte[] bytes, string password = null)
        {
            ...
            var versionStr = r.ReadStringUntilDelimiter();
            ...
            r.Pos = startXRef;
            xRef = XRef.Read(r);
            var docID = xRef.Dict["ID"]?.As<PdfArray>().To(x => new DocID(x));
            var encryptDict = (PdfDict)xRef.Dict["Encrypt"];
            if (encryptDict != null)
            {
                ...
                if (encryptDict["StmF"]?.As<PdfName>().String != "StdCF"
                        || encryptDict["StrF"]?.As<PdfName>().String != "StdCF")
                    throw new NotImplementedException("Identity crypt filter.");
                var cfm = encryptDict["CF"].As<PdfDict>()["StdCF"].As<PdfDict>()["CFM"].As<PdfName>().String;
